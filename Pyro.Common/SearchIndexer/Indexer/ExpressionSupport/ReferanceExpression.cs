...
namespace Pyro.Common.SearchIndexer.Indexer.ExpressionSupport
{
  public static class ReferanceExpression
  {
    ...
    public static Expression EqualTo_ByURLStoreKey_ReferenceFhirIdContains_Expression<ResIndexReferenceType>(
      ...
      Expression<Func<ResIndexReferenceType, bool>> ReferenceFhirIdContainsExpression,
      ConstantExpression SearchValue_History)
    {
      ...
      //(x.organization_VersionId == "2") And (FhirIdList.Contains(x.ReferenceFhirId))
      var Expression_B = Expression.AndAlso(BinaryExpression_ResourceHistory_IsEqualTo_SearchHistory, ReferenceFhirIdContainsExpression.Body);

      //(x.organization_ServiceRootURL_StoreID == 1) And (x.organization_Type == "Orginization") AND (x.organization_VersionId == "2") And (FhirIdList.Contains(x.ReferenceFhirId))
