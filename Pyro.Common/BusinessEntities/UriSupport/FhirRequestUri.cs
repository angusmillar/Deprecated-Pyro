  ...
  {    
    ...
    private string ResolvePrimaryServiceRoot(string RequestUri)
    {
      ...
      if (RequestUri.ToLower().StartsWith(this.PrimaryServiceRootServers.OriginalString))
      {
        ...
        RequestRelativePath = RequestUri.Substring(this.PrimaryServiceRootServers.OriginalString.Count(), RequestUri.Count() - this.PrimaryServiceRootServers.OriginalString.Count());
      ...
      {
        //If not and the URL starts with 'http' ot 'https' then loop through each segment of the URL looking for 
