...
{
  public class SetSearchParameterDefinitions : ISetSearchParameterDefinitions
  {
    ...
    public SetSearchParameterDefinitions(
      Devices.IPyroFhirServer IPyroFhirServerDevice, 
    ...
    private static string ResourceId = "set-searchParameter-definitions";
    ...
    public DateTimeOffset MasterLastUpdated => new DateTimeOffset(2018, 09, 05, 18, 53, 00, new TimeSpan(8, 0, 0));
    ...
    public FhirModel.Task GetResource()
    {
      ...
      FhirModel.Coding TaskCodingType = new FhirModel.Coding(IPyroTask.GetSystem(), IPyroTask.GetCode(CodeSystems.PyroTask.Codes.SetSearchParameterDefinitions));
      Resource.Code.Coding.Add(TaskCodingType);

      Resource.Description = "This task is used by the Pyro FHIR Server to load and set the base SearchParameter Definition resources as active search indexes in the FHIR server instance on the first ever start-up.";
      ...
      Resource.ReasonCode = new FhirModel.CodeableConcept() { Text = "Set active SearchParameter resources into the FHIR server instance" };
