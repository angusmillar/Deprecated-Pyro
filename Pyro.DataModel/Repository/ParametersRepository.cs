...
namespace Pyro.DataModel.Repository
{
  public partial class ParametersRepository : CommonRepository, IResourceRepository
  {
    ...
    {
      var Predicate = PredicateGenerator<Res_Parameters>(DtoSearchParameters);
      int TotalRecordCount = DbGetALLCount<Res_Parameters>(Predicate);
      var Query = DbGetAll<Res_Parameters>(Predicate);
    ...
    public IDatabaseOperationOutcome GetResourceByFhirID(string FhirResourceId, bool WithXml = false)
    {
      ...
      if (WithXml)
      {        
        DtoResource = DbGetAll<Res_Parameters>(x => x.FhirId == FhirResourceId).Select(x => new Pyro.Common.BusinessEntities.Dto.DtoResource { FhirId = x.FhirId, IsDeleted = x.IsDeleted, IsCurrent = true, Version = x.versionId, Received = x.lastUpdated, Xml = x.XmlBlob }).SingleOrDefault();       
      }
      else
      {
        DtoResource = DbGetAll<Res_Parameters>(x => x.FhirId == FhirResourceId).Select(x => new Pyro.Common.BusinessEntities.Dto.DtoResource { FhirId = x.FhirId, IsDeleted = x.IsDeleted, IsCurrent = true, Version = x.versionId, Received = x.lastUpdated }).SingleOrDefault();        
      }
