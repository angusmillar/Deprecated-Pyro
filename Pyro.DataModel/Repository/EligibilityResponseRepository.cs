...
namespace Pyro.DataModel.Repository
{
  public partial class EligibilityResponseRepository : CommonRepository, IResourceRepository
  {
    ...
    {
      var Predicate = PredicateGenerator<Res_EligibilityResponse>(DtoSearchParameters);
      int TotalRecordCount = DbGetALLCount<Res_EligibilityResponse>(Predicate);
      var Query = DbGetAll<Res_EligibilityResponse>(Predicate);
    ...
    public IDatabaseOperationOutcome GetResourceByFhirID(string FhirResourceId, bool WithXml = false)
    {
      ...
      if (WithXml)
      {        
        DtoResource = DbGetAll<Res_EligibilityResponse>(x => x.FhirId == FhirResourceId).Select(x => new Pyro.Common.BusinessEntities.Dto.DtoResource { FhirId = x.FhirId, IsDeleted = x.IsDeleted, IsCurrent = true, Version = x.versionId, Received = x.lastUpdated, Xml = x.XmlBlob }).SingleOrDefault();       
      }
      else
      {
        DtoResource = DbGetAll<Res_EligibilityResponse>(x => x.FhirId == FhirResourceId).Select(x => new Pyro.Common.BusinessEntities.Dto.DtoResource { FhirId = x.FhirId, IsDeleted = x.IsDeleted, IsCurrent = true, Version = x.versionId, Received = x.lastUpdated }).SingleOrDefault();        
      }
