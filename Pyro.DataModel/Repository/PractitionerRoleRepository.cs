...
namespace Pyro.DataModel.Repository
{
  public partial class PractitionerRoleRepository : CommonRepository, IResourceRepository
  {
    ...
    {
      var Predicate = PredicateGenerator<Res_PractitionerRole>(DtoSearchParameters);
      int TotalRecordCount = DbGetALLCount<Res_PractitionerRole>(Predicate);
      var Query = DbGetAll<Res_PractitionerRole>(Predicate);
    ...
    public IDatabaseOperationOutcome GetResourceByFhirID(string FhirResourceId, bool WithXml = false)
    {
      ...
      if (WithXml)
      {        
        DtoResource = DbGetAll<Res_PractitionerRole>(x => x.FhirId == FhirResourceId).Select(x => new Pyro.Common.BusinessEntities.Dto.DtoResource { FhirId = x.FhirId, IsDeleted = x.IsDeleted, IsCurrent = true, Version = x.versionId, Received = x.lastUpdated, Xml = x.XmlBlob }).SingleOrDefault();       
      }
      else
      {
        DtoResource = DbGetAll<Res_PractitionerRole>(x => x.FhirId == FhirResourceId).Select(x => new Pyro.Common.BusinessEntities.Dto.DtoResource { FhirId = x.FhirId, IsDeleted = x.IsDeleted, IsCurrent = true, Version = x.versionId, Received = x.lastUpdated }).SingleOrDefault();        
      }
