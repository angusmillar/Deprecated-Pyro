...
namespace Blaze.DataModel.DatabaseModel
{
  public class Res_Encounter_Index_location_Configuration : EntityTypeConfiguration<Res_Encounter_Index_location>
  {
    ...
    {
      HasKey(x => x.Res_Encounter_Index_locationID).Property(x => x.Res_Encounter_Index_locationID).IsRequired();
      Property(x => x.VersionId).IsOptional();
      Property(x => x.FhirId).IsRequired();
      Property(x => x.Type).IsRequired();
      HasRequired(x => x.Url);
      HasRequired<ServiceRootURL_Store>(x => x.Url).WithMany().HasForeignKey(x => x.ServiceRootURL_StoreID);
      HasRequired(x => x.Res_Encounter).WithMany(x => x.location_List).WillCascadeOnDelete(true);
    }
  }
}
