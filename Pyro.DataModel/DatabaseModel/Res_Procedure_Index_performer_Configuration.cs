...
namespace Pyro.DataModel.DatabaseModel
{
  public class Res_Procedure_Index_performer_Configuration : EntityTypeConfiguration<Res_Procedure_Index_performer>
  {
    ...
    {
      HasKey(x => x.Res_Procedure_Index_performerID).Property(x => x.Res_Procedure_Index_performerID).IsRequired();
      Property(x => x.VersionId).IsOptional();
      Property(x => x.FhirId).IsRequired();
      Property(x => x.Type).IsRequired();
      HasRequired(x => x.Url);
      HasRequired<ServiceRootURL_Store>(x => x.Url).WithMany().HasForeignKey(x => x.ServiceRootURL_StoreID);
      HasRequired(x => x.Res_Procedure).WithMany(x => x.performer_List).WillCascadeOnDelete(true);
    }
  }
}
