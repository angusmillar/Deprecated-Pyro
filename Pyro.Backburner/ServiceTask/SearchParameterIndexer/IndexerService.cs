...
namespace Pyro.Backburner.ServiceTask.SearchParameterIndexer
{
  public class IndexerService : IIndexerService
  {
    ...
    public void Run(ITaskPayloadPyroServerIndexing TaskPayloadPyroServerIndexing)
    {
      this._TaskPayloadPyroServerIndexing = TaskPayloadPyroServerIndexing;

      ILog.Info($"Received Task Type: {_TaskPayloadPyroServerIndexing.TaskType.GetPyroLiteral()}, Resource: Task/{_TaskPayloadPyroServerIndexing.TaskId}");
      ...
      {
        ILog.Info($"Task Type: {_TaskPayloadPyroServerIndexing.TaskType.GetPyroLiteral()}, Resource: Task/{_TaskPayloadPyroServerIndexing.TaskId}, no work to be performed.");
        ILog.Info($"Completed Task Type: {_TaskPayloadPyroServerIndexing.TaskType.GetPyroLiteral()}, Resource: Task/{_TaskPayloadPyroServerIndexing.TaskId}");
        ConsoleSupport.TimeStampWriteLine(LogMessageSupport.TaskOutCome(_TaskPayloadPyroServerIndexing, Task.TaskStatus.Completed));
      }
      else
      {
          ...
          else
          {
            ...
            using (DbContextTransaction Transaction = IUnitOfWork.BeginTransaction())
            {
              try
              {
                ...
                {
                  ILog.Info($"Task Type: {_TaskPayloadPyroServerIndexing.TaskType.GetPyroLiteral()}, Resource: Task/{_TaskPayloadPyroServerIndexing.TaskId}, Finished indexing Search Parameter: Resource: {TaskItem.ResourceType}, Name: {TaskItem.SearchParameterName}, _SearchParamId: {TaskItem.SearchParamTableId}.");                  
              ...
              {
                _ErrorDetected = true;
                _ErrorMessage = $"FHIR Task ID: {_TaskPayloadPyroServerIndexing.TaskId} of type {_TaskPayloadPyroServerIndexing.TaskType.GetPyroLiteral()} has failed. " +
        ...
        {          
          ...
          ConsoleSupport.TimeStampWriteLine(LogMessageSupport.TaskOutCome(_TaskPayloadPyroServerIndexing, Task.TaskStatus.Failed));
          ILog.Info($"Failed Task Type: {_TaskPayloadPyroServerIndexing.TaskType.GetPyroLiteral()}, Resource: Task/{_TaskPayloadPyroServerIndexing.TaskId}");
        }
      }
    }
      ...
      using (DbContextTransaction Transaction = IUnitOfWork.BeginTransaction())
      {
        try
        {
          ...
          else
          {
            ...
            IRequestMeta RequestMeta = IRequestMetaFactory.CreateRequestMeta().Set($"{ResourceType.Task.GetLiteral()}?code={Common.PyroHealthFhirResource.CodeSystems.PyroTask.System}|{Common.PyroHealthFhirResource.CodeSystems.PyroTask.Codes.SearchParameterIndexing.GetPyroLiteral()}&" +
            ...
            if (ResourceServiceOutcome.HttpStatusCode == System.Net.HttpStatusCode.OK && ResourceServiceOutcome.ResourceResult is Bundle IndexTaskBundle)
            {
              ...
              else if (IndexTaskBundle.Total > 1)
              {
                ...
                ILog.Error("Error detected by background search parameter indexer service. More than one Fhir Task with " +
        ...
        {
          ILog.Error(Exec, "Error in obtaining or setting Task status");
        }
      }
    ...
    private string FormatLogMessage(ITaskPayloadPyroServerIndexing TaskPayloadPyroServerIndexing)
    {
      if (TaskPayloadPyroServerIndexing.TaskId == null)
        return $"Run on start-up, Task Type: {TaskPayloadPyroServerIndexing.TaskType.GetPyroLiteral()}";
      else
        return $"Received Task Type: {TaskPayloadPyroServerIndexing.TaskType.GetPyroLiteral()}, Resource: Task/{TaskPayloadPyroServerIndexing.TaskId}";
    }
