...
namespace Blaze.Web.BlazeService
{
  public static class PrimaryServiceRootFactory
  {
    ...
    {
      IFhirUri RequestFhirUri = new Blaze.Common.BusinessEntities.UriSupport.DtoFhirUri(RequestUri);      
      ...
      else
      {
        if (PrimaryRootUrlStore.RootUrl != RequestFhirUri.ServiceRootUrlForComparison)
        {
          ...
          string ErrorMsg = "The incoming Http request had a service root URL of: '{0}' which is different to primary service root URL stored in the services database. The database was expecting: '{1}'. " +
                            ...
                            "You have two choices moving forward. In the database table named 'Blaze_RootUrlStore' you can locate the record with 'IsServersPrimaryUrlRoot = ture'. " +
          ...
          throw new DtoBlazeException(System.Net.HttpStatusCode.InternalServerError, OpOutCome, ErrorMsg);          
        }
      }
