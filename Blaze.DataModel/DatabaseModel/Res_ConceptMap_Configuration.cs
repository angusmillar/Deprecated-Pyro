...
namespace Blaze.DataModel.DatabaseModel
{
  public class Res_ConceptMap_Configuration : EntityTypeConfiguration<Res_ConceptMap>
  {
    ...
    public Res_ConceptMap_Configuration()
    {
      ...
      Property(x => x.source_uri_VersionId).IsOptional();
      Property(x => x.source_uri_FhirId).IsOptional();
      Property(x => x.source_uri_Type).IsOptional();
      HasOptional(x => x.source_uri_Url);
      HasOptional<ServiceRootURL_Store>(x => x.source_uri_Url).WithMany().HasForeignKey(x => x.source_uri_ServiceRootURL_StoreID);
      Property(x => x.status_Code).IsOptional();
      Property(x => x.status_System).IsOptional();
      Property(x => x.target_VersionId).IsOptional();
      Property(x => x.target_FhirId).IsOptional();
      Property(x => x.target_Type).IsOptional();
      HasOptional(x => x.target_Url);
      HasOptional<ServiceRootURL_Store>(x => x.target_Url).WithMany().HasForeignKey(x => x.target_ServiceRootURL_StoreID);
      Property(x => x.target_uri_VersionId).IsOptional();
      Property(x => x.target_uri_FhirId).IsOptional();
      Property(x => x.target_uri_Type).IsOptional();
      HasOptional(x => x.target_uri_Url);
      HasOptional<ServiceRootURL_Store>(x => x.target_uri_Url).WithMany().HasForeignKey(x => x.target_uri_ServiceRootURL_StoreID);
