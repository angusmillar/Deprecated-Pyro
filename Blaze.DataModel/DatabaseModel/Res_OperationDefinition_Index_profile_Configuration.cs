...
namespace Blaze.DataModel.DatabaseModel
{
  public class Res_OperationDefinition_Index_profile_Configuration : EntityTypeConfiguration<Res_OperationDefinition_Index_profile>
  {
    ...
    {
      HasKey(x => x.Res_OperationDefinition_Index_profileID).Property(x => x.Res_OperationDefinition_Index_profileID).IsRequired();
      Property(x => x.Uri).IsRequired();
      HasRequired(x => x.Res_OperationDefinition).WithMany(x => x.profile_List).WillCascadeOnDelete(true);
    }
  }
}
