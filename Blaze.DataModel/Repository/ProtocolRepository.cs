...
namespace Blaze.DataModel.Repository
{
  public partial class ProtocolRepository : CommonRepository, IResourceRepository
  {
    ...
    {
      var Predicate = PredicateGenerator<Res_Protocol>(DtoSearchParameters);
      int TotalRecordCount = DbGetALLCount<Res_Protocol>(Predicate);
      var Query = DbGetAll<Res_Protocol>(Predicate);
    ...
    public IDatabaseOperationOutcome GetResourceByFhirIDAndVersionNumber(string FhirResourceId, string ResourceVersionNumber)
    {
      IDatabaseOperationOutcome DatabaseOperationOutcome = new DatabaseOperationOutcome();
      DatabaseOperationOutcome.SingleResourceRead = true;
      var ResourceHistoryEntity = DbGet<Res_Protocol_History>(x => x.FhirId == FhirResourceId && x.versionId == ResourceVersionNumber);
    ...
    private void ResetResourceEntity(Res_Protocol ResourceEntity)
    {
      ...
      _Context.Res_Protocol_Index__profile.RemoveRange(ResourceEntity._profile_List);            
      _Context.Res_Protocol_Index__security.RemoveRange(ResourceEntity._security_List);            
      _Context.Res_Protocol_Index__tag.RemoveRange(ResourceEntity._tag_List);            
 
    }

    private void PopulateResourceEntity(Res_Protocol ResourseEntity, string ResourceVersion, Protocol ResourceTyped, IDtoFhirRequestUri FhirRequestUri)
    {
      ...
      if (ResourceTyped.Identifier != null)
      {
        foreach (var item3 in ResourceTyped.Identifier)
        {
          if (item3 is Hl7.Fhir.Model.Identifier)
          {
            ...
            Index = IndexSetterFactory.Create(typeof(TokenIndex)).Set(item3, Index) as Res_Protocol_Index_identifier;
      ...
      if (ResourceTyped.Meta != null)
      {
        if (ResourceTyped.Meta.Profile != null)
        {
          foreach (var item4 in ResourceTyped.Meta.ProfileElement)
          {
            ...
            {
              var Index = new Res_Protocol_Index__profile();
              Index = IndexSetterFactory.Create(typeof(UriIndex)).Set(item4, Index) as Res_Protocol_Index__profile;
      ...
      if (ResourceTyped.Meta != null)
      {
        if (ResourceTyped.Meta.Security != null)
        {
          foreach (var item4 in ResourceTyped.Meta.Security)
          {
            ...
            {
              var Index = new Res_Protocol_Index__security();
              Index = IndexSetterFactory.Create(typeof(TokenIndex)).Set(item4, Index) as Res_Protocol_Index__security;
      ...
      if (ResourceTyped.Meta != null)
      {
        if (ResourceTyped.Meta.Tag != null)
        {
          foreach (var item4 in ResourceTyped.Meta.Tag)
          {
            if (item4 is Hl7.Fhir.Model.Coding)
            {
              var Index = new Res_Protocol_Index__tag();
              Index = IndexSetterFactory.Create(typeof(TokenIndex)).Set(item4, Index) as Res_Protocol_Index__tag;
