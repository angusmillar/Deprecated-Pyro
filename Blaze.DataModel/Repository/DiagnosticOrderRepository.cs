...
namespace Blaze.DataModel.Repository
{
  public partial class DiagnosticOrderRepository : CommonRepository, IResourceRepository
  {
    ...
    {
      var Predicate = PredicateGenerator<Res_DiagnosticOrder>(DtoSearchParameters);
      int TotalRecordCount = DbGetALLCount<Res_DiagnosticOrder>(Predicate);
      var Query = DbGetAll<Res_DiagnosticOrder>(Predicate);
    ...
    private void ResetResourceEntity(Res_DiagnosticOrder ResourceEntity)
    {
      ...
      _Context.Res_DiagnosticOrder_Index_specimen.RemoveRange(ResourceEntity.specimen_List);            
      _Context.Res_DiagnosticOrder_Index__profile.RemoveRange(ResourceEntity._profile_List);            
      _Context.Res_DiagnosticOrder_Index__security.RemoveRange(ResourceEntity._security_List);            
      _Context.Res_DiagnosticOrder_Index__tag.RemoveRange(ResourceEntity._tag_List);            
      ...
      if (ResourceTyped.Meta != null)
      {
        if (ResourceTyped.Meta.Profile != null)
        {
          foreach (var item4 in ResourceTyped.Meta.ProfileElement)
          {
            ...
            {
              var Index = new Res_DiagnosticOrder_Index__profile();
              Index = IndexSetterFactory.Create(typeof(UriIndex)).Set(item4, Index) as Res_DiagnosticOrder_Index__profile;
      ...
      if (ResourceTyped.Meta != null)
      {
        if (ResourceTyped.Meta.Security != null)
        {
          foreach (var item4 in ResourceTyped.Meta.Security)
          {
            ...
            {
              var Index = new Res_DiagnosticOrder_Index__security();
              Index = IndexSetterFactory.Create(typeof(TokenIndex)).Set(item4, Index) as Res_DiagnosticOrder_Index__security;
      ...
      if (ResourceTyped.Meta != null)
      {
        if (ResourceTyped.Meta.Tag != null)
        {
          foreach (var item4 in ResourceTyped.Meta.Tag)
          {
            if (item4 is Hl7.Fhir.Model.Coding)
            {
              var Index = new Res_DiagnosticOrder_Index__tag();
              Index = IndexSetterFactory.Create(typeof(TokenIndex)).Set(item4, Index) as Res_DiagnosticOrder_Index__tag;
